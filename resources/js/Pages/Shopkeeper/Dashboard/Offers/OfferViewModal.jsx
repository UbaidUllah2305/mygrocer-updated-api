// src/Components/Admin/OfferViewModal.jsx
import React from "react";

const OfferViewModal = ({ isOpen, offer, onClose }) => {
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-[1000px] max-h-[95vh] rounded-xl bg-white shadow-xl ring-1 ring-black/10 overflow-hidden">
        <button
          type="button"
          aria-label="Close"
          onClick={onClose}
            ...
            <div className="space-y-4">
              <div className="bg-[#F4F4F4] rounded-lg p-3 sm:p-4">
                ...
                <div className="space-y-1.5 text-base text-[#3a3e47]">
                  <div><span className="font-medium">Offer Name:</span> {offer.name}</div>
                  <div><span className="font-medium">Status:</span> {offer.status}</div>
                  ...
                  <div className="ml-2 mt-1">
                    <div><span className="font-medium">Start Date:</span> {offer.schedule.split('\n')[0]?.split('–')[0]}</div>
                    <div><span className="font-medium">End Date:</span> {offer.schedule.split('\n')[0]?.split('–')[1]}</div>
                  </div>
                </div>
              </div>
