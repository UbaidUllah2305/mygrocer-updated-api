// src/Components/Admin/OverheadsDeleteModal.jsx
import React from "react";
import { Trash2, X, AlertTriangle } from "lucide-react";

const OverheadsDeleteModal = ({ isOpen, item, onConfirm, onCancel }) => {
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="overhead-delete-modal-title"
          aria-describedby="overhead-delete-modal-description"
        >
          {/* Close Button */}
          <button
            type="button"
          ...
          <div className="text-center">
            <h3 
              id="overhead-delete-modal-title"
              className="text-xl font-bold text-gray-900 mb-2"
            >
              Delete Overhead?
            </h3>
            <div id="overhead-delete-modal-description">
              <p className="text-sm text-gray-600 mb-1">
                Are you sure you want to delete the overhead
              </p>
              <p className="font-semibold text-gray-900 mb-1">
                "{item.exp}"
              </p>
              <p className="text-xs text-gray-500 mb-3">
                Amount: {item.amt} • {item.date}
              </p>
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                <p className="text-xs text-red-800 flex items-start gap-2">
                  <Trash2 className="w-4 h-4 flex-shrink-0 mt-0.5" />
                  <span>
                    This action cannot be undone. The overhead record will be permanently removed.
                  </span>
                </p>
              </div>
            </div>
...
export default OverheadsDeleteModal;