...
const OffersTable = ({ offers, onView }) => {
  if (offers.length === 0) {
    return (
        ...
        <tbody>
          {offers.map((offer, index) => (
            ...
            >
              ...
              <td className="p-4 text-center">{offer.storeName}</td>
              <td className="p-4 text-center">{offer.type}</td>
              <td className="p-4 text-center">{offer.title}</td>
              <td className="p-4 text-center">{offer.startDate}</td>
              <td className="p-4 text-center">{offer.endDate}</td>
              <td className={`p-4 text-center ${offer.status === "Read" ? "text-[#6F9C3D]" : "text-red-700"}`}>
                {offer.status}
              </td>
